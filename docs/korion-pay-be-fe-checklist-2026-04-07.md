...
| `현장 승인` / `정산 완료` 상태 라벨 통일을 위한 status/reason 구분 | ✅ BE-1에서 `status`, `sagaStep`, `senderHistoryStatus`, `receiverHistoryStatus` 분리 |
| 수신자 미확인 만료 추적 | ✅ `receiverConfirmationDeadlineAt`, `receiverConfirmationExpiredAt`, `receiverConfirmationExpired` 포함 |
| 확정 후 proof 충돌 추적 | ✅ BE scanner가 `POST_FINAL_PROOF_CONFLICT` reconciliation case 생성 |
| 확정 후 proof 충돌 운영자 액션 | ✅ `request-compensation`, `resolve-no-compensation`, `close` admin API 및 admin 화면 사유 입력 액션 반영 |

**FE 진행 가능 범위**: ledger 결과, sender/receiver leg 상태, 수신자 미확인 만료 상태 기반 문구 표시 가능.
...
| settlement detail에서 receiver leg 상태 명시 노출 | ✅ `receiverHistoryStatus` 및 수신자 확인 만료 필드 포함 |

**FE 진행 가능 범위**: receiver history 조회, 수취 확인 완료 화면, receiver 상세 상태/만료 상태 표시 가능.
