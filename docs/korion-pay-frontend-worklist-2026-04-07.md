...
- 해야 할 일:
  ...
  - [x] 받은쪽 `Settle now` / `Auto settle` 실행 시 완료된 수취 합계는 wallet/history 반영 확인에만 사용하고 담보 채우기 큐를 만들지 않음
  - [x] wallet/history 반영이 확인된 received item id를 로컬 settled marker로 저장해 동일 수취 내역 중복 정산 표시를 방지
  - [x] 받은쪽 총 수취금액은 KORION wallet(Foxya) 반영 확인 후 로컬 표시에서 제외해 `정산하기를 누르면 수취 합계는 리셋` 문구와 일치시킴
